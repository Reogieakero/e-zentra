...
Stack: Node.js + TypeScript, Express, Prisma + PostgreSQL, Upstash Redis (cache/rate limiting), JWT auth (rotated
hashed refresh tokens), Zod validation, pino logging, Jest integration tests.
...
`POST /api/v1/uploads/:kind`, stored under `UPLOAD_DIR` (default `./uploads`), and served statically at
`/uploads/...`. Parent-student linking is exposed at `/api/v1/parent-links`, and report cards can be
auto-generated from final grades via `POST /api/v1/report-cards/generate`.
